...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.FaceRecButton = customtkinter.CTkButton(
            self, text="Face Recognition", command=self.start_face_recognition, width=200
        )
        self.FaceRecButton.grid(row=3, column=1, padx=20, pady=20)

        self.NFCButton = customtkinter.CTkButton(
            self, text="NFC Read", command=lambda: print("NFC module not available"), width=200
        )
        self.NFCButton.grid(row=3, column=2, padx=20, pady=20)
    ...
    def start_face_recognition(self):
        face_rec_thread = threading.Thread(target=self.face_recognition_process, daemon=True)
        face_rec_thread.start()

    def start_nfc_reading(self):
        nfc_thread = threading.Thread(target=self.nfc_reading_process, daemon=True)
        nfc_thread.start()
    ...
    def face_recognition_process(self):
        try:
            ...
            for user in users:
                if "FaceEncoding" in user:
                    known_face_encodings.append(user["FaceEncoding"])
                    known_face_names.append(user["Item"])
    ...
    def nfc_reading_process(self):
        try:
            with nfc.ContactlessFrontend("usb") as clf:
                while True:
                    tag = clf.connect(rdwr={"on-connect": lambda tag: False})
                    print(f"NFC Tag UID: {tag.identifier.hex()}")
                    self.write_to_log("NFC Read", f"Read NFC Tag {tag.identifier.hex()}")
        except Exception as e:
            print(f"Error in NFC reading process: {e}")
